...
// 假設像素與實際距離比例為 1:1（後續可調整）
const PIXEL_TO_METER = 0.0075;

// markers: [{x, y, time, type}]
export function calcSpeeds(markers) {
  const results = [];
  for (let i = 0; i + 1 < markers.length; i += 2) {
    const hit = markers[i];
    const land = markers[i + 1];
    const distance = calcDistance(hit.x, hit.y, land.x, land.y) * PIXEL_TO_METER;
